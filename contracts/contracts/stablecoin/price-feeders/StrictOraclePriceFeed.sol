...
contract StrictOraclePriceFeed is PausableUpgradeable, AccessControlUpgradeable, IPriceFeed {
  using SafeMathUpgradeable for uint256;

  struct TauOracleConfig {
    IOracle alpacaOracle;
    address token0;
    address token1;
  }

  // primary.alpacaOracle will be use as the price source
  TauOracleConfig public primary;
  // secondary.alpacaOracle will be use as the price ref for diff checking
  TauOracleConfig public secondary;
  ...
  // --- Init ---
  function initialize(
    address _primaryTauOracle,
    address _primaryToken0,
    address _primaryToken1,
    address _secondaryTauOracle,
    ...
    primary.alpacaOracle = IOracle(_primaryTauOracle);
    primary.token0 = _primaryToken0;
    primary.token1 = _primaryToken1;

    secondary.alpacaOracle = IOracle(_secondaryTauOracle);
  ...
  event LogSetPrimary(address indexed caller, address newTauOracle, address primaryToken0, address primaryToken1);
  event LogSetSecondary(
    address indexed caller,
    address newTauOracle,
  ...
  function setPriceLife(uint256 _second) external onlyOwner {
    require(_second >= 1 hours && _second <= 1 days, "StrictOraclePriceFeed/bad-price-life");
    priceLife = _second;
  ...
  function setPrimary(
    ...
    primary.token1 = _primaryToken1;
    primary.alpacaOracle.getPrice(primary.token0, primary.token1);
    emit LogSetPrimary(msg.sender, _primaryTauOracle, _primaryToken0, _primaryToken1);
  }
  ...
  function setSecondary(
    ...
    secondary.token1 = _secondaryToken1;
    secondary.alpacaOracle.getPrice(secondary.token0, secondary.token1);
    emit LogSetSecondary(msg.sender, _secondaryTauOracle, _secondaryToken0, _secondaryToken1);
  }
