...
contract OraclePriceFeed is PausableUpgradeable, IPriceFeed {
  ...
  function initialize(
    ...
    require(_token0 != _token1, "OraclePriceFeed/wrong-token0-token1");
    token0 = _token0;
    token1 = _token1;
    priceLife = 1 days;
  }
  ...
  function setPriceLife(uint256 _second) external onlyOwner {
    require(_second >= 1 hours && _second <= 1 days, "OraclePriceFeed/bad-price-life");
    priceLife = _second;
  ...
  function readPrice() external view override returns(bytes32) {
    (uint256 _price, ) = oracle.getPrice(token0, token1);
    return bytes32(_price);
  }

  function peekPrice() external view override returns(bytes32, bool) {
    // [wad], [seconds]
    (uint256 _price, uint256 _lastUpdate) = oracle.getPrice(token0, token1);
