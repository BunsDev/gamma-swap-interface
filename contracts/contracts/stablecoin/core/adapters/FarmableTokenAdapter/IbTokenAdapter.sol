...
/// It receives Tau's ibTOKEN from users and deposit in Tau's Launch.
/// Hence, users will still earn TAU rewards while holding positions.
contract IbTokenAdapter is IFarmableTokenAdapter, PausableUpgradeable, ReentrancyGuardUpgradeable, ICagable {
  ...
  /// @dev The Tau's Fairlaunch contract
  ILaunch public fairlaunch;
  /// @dev The Tau's Shield contract
  ...
  function initialize(
    ...
    // 2. Sanity checks
    (address _stakeToken, , , , ) = ILaunch(_fairlaunch).poolInfo(_pid);
    require(_stakeToken == _collateralToken, "IbTokenAdapter/collateralToken-not-match");
    require(ILaunch(_fairlaunch).alpaca() == _rewardToken, "IbTokenAdapter/reward-token-not-match");
    require(ILaunch(_fairlaunch).owner() == _shield, "IbTokenAdapter/shield-not-match");
    require(IShield(_shield).owner() == _timelock, "IbTokenAdapter/timelock-not-match");

    fairlaunch = ILaunch(_fairlaunch);
  ...
  function _harvestFromFarm() internal returns(uint256) {
    if(live == 1) {
      ...
      uint256 _pendingTau = fairlaunch.pendingTau(pid, address(this));
      (uint256 _stakedBalance, , , ) = fairlaunch.userInfo(pid, address(this));
      if(_stakedBalance > 0 && _pendingTau > 0) fairlaunch.withdraw(address(this), pid, 0);
    }
  ...
  /// @param _positionAddress The address that you want to check pending TAU
  function pendingRewards(address _positionAddress) external view returns(uint256) {
    return _pendingRewards(_positionAddress, fairlaunch.pendingTau(pid, address(this)));
  }
  ...
  function netPendingRewards(address _positionAddress) external view returns(uint256) {
    return _netPendingRewards(_positionAddress, fairlaunch.pendingTau(pid, address(this)));
  }
  ...
  /// @dev Harvest and deposit received ibToken to Launch
  /// @param _positionAddress The address that holding states of the position
  /// @param _amount The ibToken amount that being used as a collateral and to be deposited to Launch
  ...
  /// @dev Harvest and withdraw ibToken from Launch
  /// @param _usr The address that holding states of the position
  /// @param _amount The ibToken amount to be withdrawn from Launch and return to user
  function withdraw(
    address _usr,
    uint256 _amount,
    bytes calldata /* _data */
  ...
  /// @dev EMERGENCY ONLY. Withdraw ibToken from Launch with invoking "_harvest"
