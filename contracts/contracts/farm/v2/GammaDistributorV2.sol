...
import "./interfaces/IGammaPair.sol";


contract GammaDistributorV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTimestamp; // Last block number that Tau distribution occurs.
        uint256 accTauPerShare; // Accumulated Tau per share, times 1e18. See below.
    ...
    IBoringERC20 public tau;

    // Tau tokens created per second
    uint256 public tauPerSec;
    ...
    // The timestamp when Tau mining starts.
        ...
        )
    {
        ...
        if(block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accTauPerShare += (
                ((tauReward * ACC_TOKEN_PRECISION) / lpSupply)
            );
        }

        uint256 pendingTau = (((user.amount * accTauPerShare) / ACC_TOKEN_PRECISION) - user.rewardDebt) + user.rewardLockedUp;
        ...
        addresses[0] = address(tau);
        symbols[0] = IBoringERC20(tau).safeSymbol();
        decimals[0] = IBoringERC20(tau).safeDecimals();
        amounts[0] = pendingTau;
        ...
        )
    {
        ...
        rewardsPerSec[0] =
            (pool.allocPoint * tauPerSec * lpPercent) /
            totalAllocPoint /
            total;
    ...
    function _updatePool(uint256 _pid) internal validatePoolByPid(_pid) {
        ...
        uint256 tauReward = ((multiplier * tauPerSec) * pool.allocPoint) / totalAllocPoint;
        ...
        tau.mint(teamAddress, (tauReward * teamPercent) / total);
        tau.mint(treasuryAddress, (tauReward * treasuryPercent) / total);
        tau.mint(investorAddress, (tauReward * investorPercent) / total);
        tau.mint(address(this), (tauReward * lpPercent) / total);

        pool.accTauPerShare +=
            (tauReward * ACC_TOKEN_PRECISION * lpPercent) /
            pool.totalLp /
            total;
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        IGammaPair pair = IGammaPair(address(pool.lpToken));
        pair.permit(msg.sender, address(this), amount, deadline, v, r, s);
        _deposit(pid, amount);
    }

    // Deposit tokens for Tau allocation.
        ...
        validatePoolByPid(_pid)
    {
        ...
        if(_amount > 0) {
            ...
            if(address(pool.lpToken) == address(tau)) {
                totalTauInPools += _amount;
            }
        }

        user.rewardDebt = (user.amount * pool.accTauPerShare) / ACC_TOKEN_PRECISION;
        ...
        for(
            ...
            pool.rewarders[rewarderId].onTauReward(
                _pid,
                msg.sender,
                user.amount
            );
        ...
        validatePoolByPid(_pid)
    {
        ...
        user.rewardDebt =
            (user.amount * pool.accTauPerShare) /
            ACC_TOKEN_PRECISION;
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        for(
            ...
            pool.rewarders[rewarderId].onTauReward(_pid, msg.sender, 0);
        }

        if(address(pool.lpToken) == address(tau)) {
            totalTauInPools -= amount;
        }
    ...
    function payOrLockupPendingTau(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = ((user.amount * pool.accTauPerShare) /
        ...
        if(canHarvest(_pid, msg.sender)) {
            if(pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeTauTransfer(msg.sender, pendingRewards);
            }
        } else if(pending > 0) {
    ...
    function safeTauTransfer(address _to, uint256 _amount) internal {
        if(tau.balanceOf(address(this)) > totalTauInPools) {
            ...
            uint256 tauBal = tau.balanceOf(address(this)) -
                totalTauInPools;
            if(_amount >= tauBal) {
                tau.safeTransfer(_to, tauBal);
            } else if(_amount > 0) {
                tau.safeTransfer(_to, _amount);
            }
        }
    }


    function updateEmissionRate(uint256 _tauPerSec) public onlyOwner {
        _massUpdatePools();

        emit EmissionRateUpdated(msg.sender, tauPerSec, _tauPerSec);
