...
async function validateCollateralPool(collateralPoolConfig: CollateralPoolConfig, collateralPoolInfo: CollateralPool) {
    ...
    try {
        expect(collateralPool.debtCeiling.toString()).to.be.equal(
            collateralPoolInfo.debtCeiling,
            "debtCeiling mis-config"
        )
...
async function validateStrictOraclePriceFeed(priceFeedInfo: StrictAlpacaPriceOraclePriceFeed) {
    const priceFeed = StrictOraclePriceFeed__factory.connect(priceFeedInfo.address, ethers.provider)
    try {
...
async function validateRole(
    ...
    try {
        ...
        // MINTER_ROLE
        const AUSD = TauStablecoin__factory.connect(config.TauStablecoin.AUSD.address, ethers.provider)
...
async function main() {
    ...
    console.log("=== validate StrictOraclePriceFeed config ===")
    const validateStrictOraclePriceFeeds = []
    for (const strictAlpacaPriceOraclePriceFeed of config.PriceFeed.StrictAlpacaPriceOraclePriceFeed) {
        validateStrictOraclePriceFeeds.push(validateStrictOraclePriceFeed(strictAlpacaPriceOraclePriceFeed))
    }
    await Promise.all(validateStrictOraclePriceFeeds)
