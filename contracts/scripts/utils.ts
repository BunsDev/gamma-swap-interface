...
const DEPLOYMENT_FILE = path.resolve(__dirname, "../../src", "constants/deployments.json")
console.debug("Deployment file:", DEPLOYMENT_FILE)
...
export const getChainId = async (hre: HardhatRuntimeEnvironment) => {
    ...
    try {
        ...
        console.debug("Chain id from hardhat:", _chainId)
    } catch (e) {
        console.log("failed to get chainId, f alling back on net_version...")
        _chainId = await hre.network.provider.send("net_version")
        console.debug("Chain id from hardhat 2:", _chainId)
    }
...
export function getDeployments() {
    const deployments = JSON.parse(fs.readFileSync(DEPLOYMENT_FILE).toString())
    return deployments
}

export function saveDeployments(deployments: any) {
    fs.writeFileSync(DEPLOYMENT_FILE, JSON.stringify(deployments, null, 4))
}
