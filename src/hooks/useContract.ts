...
import GAMMA_DISTRIBUTOR_ABI from "../constants/abis/solar-distributor.json"
...
import GammaFactoryABI from "../constants/artifacts/contracts/swap/GammaFactory.sol/GammaFactory.json"
import GammaRouter02ABI from "../constants/artifacts/contracts/swap/GammaRouter02.sol/GammaRouter02.json"
...
export function useGammaDistributorContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && GAMMA_DISTRIBUTOR_ADDRESS[chainId], GammaDistributor.abi, withSignerIfPossible)
}

export function useGammaVaultContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && GAMMA_VAULT_ADDRESS[chainId], SOLAR_VAULT_ABI, withSignerIfPossible)
}
...
export function useSwapFactoryContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && FACTORY_ADDRESS[chainId], GammaFactoryABI.abi, withSignerIfPossible)
}
...
export function useRouterContract(useArcher = false, withSignerIfPossible?: boolean): Contract | null {
    ...
    const abi = useArcher ? ARCHER_ROUTER_ABI : GammaRouter02ABI.abi
