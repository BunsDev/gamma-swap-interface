...
const YieldDetails: React.FC<YieldDetailsProps> = ({
    ...
    const perDay: number = Number((1000 * roiPerDay) / tauPrice)
    const perWeek: number = Number((1000 * roiPerWeek) / tauPrice)
    const perMonth: number = Number((1000 * roiPerMonth) / tauPrice)
    const perYear: number = Number((1000 * roiPerYear) / tauPrice)
    ...
    const getModalContent = () => (
        <div className="space-y-6">
            <div className="space-y-2">
                <ModalHeader title={`Pool Details`} onClose={onDismiss} />
            ...
            <div className="space-y-2 flex flex-col">
                <div className="flex flex-row flex-nowrap gap-1 font-bold">
                    ...
                    <div className="flex flex-row py-1 px-2 w-full">
                        <div className="flex items-center justify-between uppercase">{`ROI`}</div>
                    </div>

                    <div className="flex flex-row py-1 px-2 w-full">
                        <div className="flex items-center justify-between uppercase">{`SOLAR per $1000`}</div>
                    </div>
                </div>
